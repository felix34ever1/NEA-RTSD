...
class Spot: # Tile like class
    ...
    # manhatten distance / taxi distance
def heuristic(p1, p2):
    x1, y1 = p1
    x2, y2 = p2
    return abs(x1 - x2) + abs(y1 - y2)

def reconstruct_path(came_from, current, draw): #traverse from end node to start node
...
def algorithm(draw,grid,start,end):
    ...
    while not open_set.empty(): #allows to close whilst running
        ...
        if current != start:
            current.make_closed() #make node red and close
